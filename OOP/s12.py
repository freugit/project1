...
#得到邮件的原始内容
#这个过程主要负责从MDA到MUA的下载，并使用Parse粗略解析

def getMsg():
    #准备相应的信息
    email = "190558283@qq.com"
    #邮箱授权码
    pwd = "ryowkcuzmjbibige"
    ...
    #可以查看返回的mails列表类似[b'1 829222',b'2 8212'...]
    print(mails)

    #获取最新一封邮件，注意，邮件索引号是从1开始，最新代表索引号最高
    index = len(mails)

    #retr负责返回一个具体索引号的一封信的内容，此内容不具可读性
...
def parseMsg(msg,indnet = 0):
    ...
    if indnet == 0:
        for header in ["From","To","Subject"]:
            ...
            print("{0},{1}:{2}".format(indnet,header,value))
    ...
    if (msg.is_multipart()):
        ...
        #enumerate函数是内置函数
        #作用是将一个列表，此处是parts，生成一个由索引和parts原内容构成的新的列表
        #例如enumerate([a,b,c])结果是：[(1,a),(2,b),(3,c)]
        for n,part in enumerate(parts):
            #一个字符串乘以一个数字的意思是对这个字符串进行n倍扩展
            #比如"aa"*2 -> "aaaa"
            print("{0}spart:{1}".format(" "*indnet,n))
            parseMsg(part,indnet+1)
    else:#基础类型
        ...
        if content_type == "text/plain" or content_type == "text/html":
            ...
            print("{0}Text:{1}".format(indnet,content))

        else:#不是文本内容，则应该是附件
            print("{0}Attachment:{1}".format(indnet,content_type))

def decodeStr(s):
    """
    s 代表一封邮件From,To,Subject中的任一项
    对s进行解码，解码是编码的逆过程
